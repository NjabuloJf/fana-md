...
async function getTranslatedTexts() {
    ...
    return {
        ...
        pleaseInsert: await translateTextWithCache("⚠️ *Please insert a Snapchat link!*", lang),
        example: await translateTextWithCache("📌 Example:", lang),
        videoReady: await translateTextWithCache("🎬 *Your video is ready!*", lang),
        snapchatVideo: await translateTextWithCache("📥 *SNAPCHAT SPOTLIGHT*", lang),
...
async function fetchSnapchatInfo(url) {
    try {
        ...
        if (response.status === 200 && response.data) {
            ...
            const result = {
                title: data.videoTitle || data.title || data.caption || "Snapchat Spotlight",
            ...
            console.log(`✅ Snapchat data parsed: Video=${result.videoUrl ? 'Yes' : 'No'}`);
            return result;
        }
...
// ========== CREATE CARDS ==========
async function createSnapchatCards(mediaInfo, zk, ms, lang) {
    ...
    const title = mediaInfo.title || "Snapchat Spotlight";
...
// ========== DOWNLOAD FUNCTIONS ==========
async function downloadSnapchatVideo(zk, dest, ms, data, lang, isDocument, quality) {
    try {
        const t = await getTranslatedTexts();
        ...
        const title = data.title || "Snapchat Spotlight";
...
async function downloadSnapchatAudio(zk, dest, ms, data, lang) {
    try {
        const t = await getTranslatedTexts();
        ...
        const title = data.title || "Snapchat Audio";
...
fana({
    ...
    if (replyText && isNumberSelection(replyText)) {
        ...
        switch(selectedNumber) {
            case 1:
                await downloadSnapchatAudio(zk, dest, ms, data, lang);
                break;
            case 2:
                await downloadSnapchatVideo(zk, dest, ms, data, lang, false, 'mp4');
                break;
            case 3:
                await downloadSnapchatVideo(zk, dest, ms, data, lang, true, 'mp4');
                break;
            case 4:
                await downloadSnapchatVideo(zk, dest, ms, data, lang, false, 'hd');
                break;
            case 5:
                await downloadSnapchatVideo(zk, dest, ms, data, lang, false, 'sd');
                break;
            default:
                await repondre(t.invalidChoice);
                return;
    ...
    if (!arg[0]) {
        return await repondre(`${t.pleaseInsert}\n\n${t.example} .snapchat https://www.snapchat.com/xxxxx`);
    }
    ...
    try {
        const result = await fetchSnapchatInfo(queryURL);
        ...
        activeDownloads[senderJid] = {
            title: result.title || "Snapchat Spotlight",
            url: queryURL,
        ...
        const { cards } = await createSnapchatCards(result, zk, ms, lang);
