...
// ========== SESSION HANDLER - SINGLE AUTH METHOD ==========
...
async function cleanInvalidSession() {
    ...
    if (fs.existsSync(credsPath)) {
        try {
            ...
            console.log('⚠️ Session file corrupted, clearing...');
            fs.unlinkSync(credsPath);
            return false;
        }
    }
...
// ========== REMOVED: loadSession() function - using single auth method only ==========

const prefixe = conf.PREFIXE || ".";
const more = String.fromCharCode(8206);
const readmore = more.repeat(4001);

// ========== REMOVED: authentification() function - using single auth method only ==========
...
setTimeout(() => {
    async function main() {
        try {
            // Clean invalid session before connecting
                        ...
                        if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
                            ...
                            if (deletedMessage) {
                                try {
                                    ...
                                    const buttons = await getTranslatedButtons(conf.LANGUAGE || "en");
            ...
            zk.ev.on("messages.upsert", async (m) => {
                try {
                    ...
                    if (error.message && error.message.includes('Bad MAC')) {
                        console.log('🔐 Decryption error detected - session may need refresh');
        ...
        *-᳆ .💌 use .setlang you owner country language*
        ...
        `;
                    ...
                    if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
                        ...
                        // Clean invalid session and restart
                        await cleanInvalidSession();
