...
async function createTikTokCards(videoInfo, zk, ms, lang) {
    ...
    // Get video details
    const title = videoInfo.title || videoInfo.desc || t.unknown;
    const author = videoInfo.author?.unique_id || videoInfo.author?.username || t.unknown;
    const duration = videoInfo.duration || '0:00';
    const views = videoInfo.views || videoInfo.play_count || 0;
    const likes = videoInfo.likes || videoInfo.digg_count || 0;
    const comments = videoInfo.comments || videoInfo.comment_count || 0;
    const shares = videoInfo.shares || videoInfo.share_count || 0;
    const thumbnail = videoInfo.cover || videoInfo.thumbnail || randomNjabulourl;
    ...
    const card1 = {
        header: {
            ...
            imageMessage: thumbnail ? 
                (await generateWAMessageContent({ image: { url: thumbnail } }, { upload: zk.waUploadToServer })).imageMessage :
            ...
            buttons: [
                {
                    ...
                    buttonParamsJson: JSON.stringify({
                        ...
                        copy_code: videoInfo.video_url || videoInfo.url || "Link not available",
                    }),
                },
                {
                    name: "cta_url",
...
fana({
    ...
    try {
        ...
        activeDownloads[senderJid] = {
            ...
            hdUrl: result.video_url_hd || result.video_url,
            sdUrl: result.video_url_sd || result.video_url,
            timestamp: Date.now()
        };
