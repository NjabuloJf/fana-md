...
setTimeout(() => {
    async function main() {
        ...
        zk.ev.on('group-participants.update', async (group) => {
            ...
            try {
                ...
                if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
                    let msg = `*FANA MD WELCOME MESSAGE*`;
        ...
        // Connection update event - FIXED with proper zk.user.id handling
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                // FIX: Wait for user.id to be available
                let waitCount = 0;
                ...
                // Send startup message - FIXED to handle missing user.id
