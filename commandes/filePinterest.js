...
async function getTranslatedTexts() {
    ...
    return {
        ...
        pleaseInsert: await translateTextWithCache("⚠️ *Please insert a Pinterest link!*", lang),
        ...
        pinterestPost: await translateTextWithCache("📥 *PINTEREST POST*", lang),
...
async function createPinterestCards(mediaInfo, zk, ms, lang) {
    ...
    const title = mediaInfo.title || "Pinterest Post";
        ...
        dest,
        {
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: {
                        body: { text: `📥 *Pinterest Downloader*` },
...
fana({
    ...
    if (replyText && isNumberSelection(replyText)) {
        ...
        switch(selectedNumber) {
            case 1:
                await downloadPinterestAudio(zk, dest, ms, data, lang);
                break;
            case 2:
                await downloadPinterestVideo(zk, dest, ms, data, lang, false);
                break;
            case 3:
                await downloadPinterestVideo(zk, dest, ms, data, lang, true);
                break;
            case 4:
                await downloadPinterestImage(zk, dest, ms, data, lang);
                break;
            case 5:
                await downloadPinterestCarousel(zk, dest, ms, data, lang);
                break;
            default:
                await repondre(t.invalidChoice);
                return;
    ...
    if (!arg[0]) {
        return await repondre(`${t.pleaseInsert}\n\n${t.example} .pinterest https://www.pinterest.com/pin/xxxxx`);
    }
    ...
    try {
        const result = await fetchPinterestInfo(queryURL);
        ...
        activeDownloads[senderJid] = {
            mediaInfo: result,
            title: result.title || "Pinterest Post",
            url: queryURL,
        ...
        const { cards } = await createPinterestCards(result, zk, ms, lang);
...
async function downloadPinterestVideo(zk, dest, ms, data, lang, isDocument) {
    try {
        const t = await getTranslatedTexts();
        ...
        const title = data.title || "Pinterest Video";
...
async function downloadPinterestImage(zk, dest, ms, data, lang) {
    try {
        const t = await getTranslatedTexts();
        let imageUrl = null;
        ...
        const title = data.title || "Pinterest Image";
...
// ========== DOWNLOAD PINTEREST CAROUSEL ==========
async function downloadPinterestCarousel(zk, dest, ms, data, lang) {
    try {
        const t = await getTranslatedTexts();
        ...
        const title = data.title || "Pinterest Carousel";
...
async function downloadPinterestAudio(zk, dest, ms, data, lang) {
    try {
        const t = await getTranslatedTexts();
        ...
        const title = data.title || "Pinterest Audio";
