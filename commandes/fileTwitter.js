...
async function getTranslatedTexts() {
    ...
    return {
        ...
        pleaseInsert: await translateTextWithCache("⚠️ *Please insert a Twitter/X link!*", lang),
        ...
        twitterPost: await translateTextWithCache("📥 *TWITTER/X POST*", lang),
...
async function fetchTwitterInfo(url) {
    try {
        ...
        if (response.status === 200 && response.data) {
            ...
            const result = {
                title: data.videoTitle || data.title || data.caption || "Twitter/X Post",
            ...
            console.log(`✅ Twitter data parsed: Video=${result.isVideo}, Images=${result.images.length}`);
            return result;
        }
    ...
    const card1 = {
        ...
        body: {
            text: `${t.title} ${title}\n` +
                  ...
                  `${t.retweets} ${retweets.toLocaleString()}\n` +
        ...
        dest,
        {
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: {
                        body: { text: `📥 *Twitter/X Downloader*` },
...
async function downloadTwitterAudio(zk, dest, ms, data, lang) {
    try {
        ...
        const title = data.title || "Twitter/X Audio";
